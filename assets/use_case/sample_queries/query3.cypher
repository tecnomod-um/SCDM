...
// get the information of the second evaluation (167 patients expected).
...
WITH patient, barthelDischargeSession, dischargeDaysSinceInjury
MATCH (barthelDischargeSession)-[:hasPart]->(itemProcedure:ClinicalProcess)-[:hasOutcome]->(cs:ClinicalStatement)-[:hasPart|:interprets_observable]->(observable:`363787002`)
WITH patient, barthelDischargeSession, dischargeDaysSinceInjury, itemProcedure, cs, observable
CALL {
      WITH patient, barthelDischargeSession, dischargeDaysSinceInjury, itemProcedure, cs, observable
...
RETURN patient.label[0] as patient, 
       barthelDischargeSession.displayLabel[0] as session, 
       dischargeDaysSinceInjury.hasValue[0] as daysSinceInjury, 
